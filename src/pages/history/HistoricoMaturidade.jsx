...
export default function HistoricoMaturidade() {
  ...
  const loadData = async () => {
    try {
      ...
      const [diagsData, empsData] = await Promise.all([
        base44.entities.CollaboratorMaturityDiagnostic.filter({ workshop_id: workshopId }, '-created_date'),
    ...
    <div className="min-h-screen bg-slate-50 p-8">
      <div className="max-w-6xl mx-auto">
        <div className="flex justify-between items-center mb-8">
          <div>
            <h1 className="text-3xl font-bold text-gray-900">Histórico de Maturidade</h1>
            <p className="text-gray-600">Acompanhe a evolução da maturidade profissional</p>
          </div>
          <Button onClick={() => navigate(createPageUrl("DiagnosticoMaturidade"))}>Novo Diagnóstico</Button>
        </div>
                ...
                <TableRow key={diag.id}>
                  ...
                  <TableCell>
                    <Button 
                      variant="outline" 
                      size="sm"
                      onClick={() => navigate(createPageUrl("ResultadoMaturidade") + `?id=${diag.id}`)}
                    >
                      Ver Resultado
                    </Button>
                  </TableCell>

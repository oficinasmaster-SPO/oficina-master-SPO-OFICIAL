...
export default function EditarDescricaoCargo() {
  ...
  const loadData = async () => {
    try {
      ...
      if (current) {
        ...
        toast.error("Descrição não encontrada");
        navigate(createPageUrl("DescricoesCargo"));
      }
  ...
  const addItem = () => {
    ...
    if (currentQuestion.type === "list-checkbox") {
      setFormData({
        ...formData,
        [field]: [...currentList, { item: tempInput.item, required: tempInput.required, desired: tempInput.desired }]
      });
    } else if (currentQuestion.type === "list-internal-external") {
      setFormData({
        ...formData,
        [field]: [...currentList, { item: tempInput.item, internal: tempInput.required, external: tempInput.desired }]
      });
      ...
      setFormData({
        ...formData,
        [field]: [...currentList, tempInput.item]
      });
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      await base44.entities.JobDescription.update(jobDescriptionId, formData);
            ...
            {currentQuestion.type === "list-indicators-advanced" && (
              <div className="space-y-4">
                ...
                <div className="border-t pt-4 mt-6">
                  <Label className="text-sm text-gray-600 mb-2 block">Indicadores Adicionados ({(formData[currentQuestion.id] || []).length})</Label>
                  ...
                  <div className="border rounded-lg overflow-hidden">
                    <table className="w-full text-sm text-left">
                      ...
                      <tbody className="divide-y divide-gray-200 bg-white">
                        {(formData[currentQuestion.id] || []).map((indicator, index) => (
                          <tr key={index}>
                        ...
                        {(!formData[currentQuestion.id] || formData[currentQuestion.id].length === 0) && (
                          <tr>
            ...
            {(currentQuestion.type === "list-simple" || currentQuestion.type === "list-checkbox" || currentQuestion.type === "list-internal-external") && (
              <div className="space-y-4">
                ...
                <div className="border-t pt-4">
                  <Label className="text-sm text-gray-600">Itens Adicionados ({(formData[currentQuestion.id] || []).length})</Label>

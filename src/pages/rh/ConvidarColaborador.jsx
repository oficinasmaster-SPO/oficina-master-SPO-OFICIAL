...
export default function ConvidarColaborador() {
  ...
  const jobRoles = [
    ...
    { value: "lider_tecnico", label: "LÃ­der TÃ©cnico" },
  ...
  // Buscar oficinas do usuÃ¡rio (nÃ£o apenas a principal)
  ...
  // Buscar perfis disponÃ­veis
  const { data: profiles = [], isLoading: isLoadingProfiles } = useQuery({
    queryKey: ['user-profiles'],
    queryFn: async () => {
      const allProfiles = await base44.entities.UserProfile.list();
      // CORRIGIDO: Busca perfis 'externo' ao invÃ©s de 'cliente'
      const filtered = allProfiles.filter(p => 
        (p.type === 'externo' || p.type === 'cliente') && 
        p.status === 'ativo'
      );
      console.log("ðŸ“‹ Perfis carregados:", filtered);
      console.log("ðŸ“‹ Total de perfis no sistema:", allProfiles.length);
      return filtered;
    }
  });

  // Preencher formulÃ¡rio com dados do Employee
  ...
  useEffect(() => {
    const loadUserAndWorkshop = async () => {
      try {
        ...
        // Busca oficina onde o usuÃ¡rio Ã© dono (mais seguro que listar todas)
        ...
        if (userWorkshop) {
          ...
          // Se tiver ID na URL, preenche o formulÃ¡rio
  ...
  // MutaÃ§Ã£o para criar colaborador
  const createUserMutation = useMutation({
    mutationFn: async (data) => {
      ...
      // Verificar se colaborador jÃ¡ existe
      ...
      if (existingEmployees && existingEmployees.length > 0) {
        ...
        const shouldResend = window.confirm(
          `Colaborador ${data.name} (${data.email}) jÃ¡ estÃ¡ cadastrado.\n\n` +
      ...
      console.log("ðŸš€ Criando novo colaborador:", data.email);
    ...
    onSuccess: (data) => {
      console.log("ðŸŽ‰ Sucesso ao criar colaborador:", data);
      console.log("ðŸ“§ Email:", data?.email);
      console.log("ðŸ”‘ Senha:", data?.temporary_password);
      console.log("ðŸ”— Invite Link:", data?.invite_link);
      
      // Validar se todos os dados necessÃ¡rios estÃ£o presentes
      if (!data?.email || !data?.temporary_password || !data?.invite_link) {
        console.error("âŒ Dados incompletos na resposta:", data);
        toast.error("Erro: Dados incompletos na resposta do servidor");
        return;
      }
      
      console.log("âœ… Dados validados e prontos para exibiÃ§Ã£o");
      console.log("ðŸ“ Setando createdUser com:", { email: data.email, password: data.temporary_password, link: data.invite_link });
      queryClient.invalidateQueries({ queryKey: ['employees-list'] });
      
      // Somente setar createdUser quando TODOS os dados estÃ£o disponÃ­veis
      setCreatedUser(data);
      console.log("âœ… createdUser setado com sucesso");

      if (data.action === 'resent') {
        toast.success("ðŸ“§ Convite reenviado com sucesso!", { duration: 5000 });
      } else {
        toast.success("âœ… Colaborador criado com sucesso!", { duration: 5000 });
      }
    ...
    onError: (error) => {
      ...
      if (error.message === "OperaÃ§Ã£o cancelada pelo usuÃ¡rio") {
        return; // NÃ£o mostrar erro se usuÃ¡rio cancelou
      }
  ...
  const handleSubmit = (e) => {
    ...
    if (!formData.workshop_id && !workshop?.id) {
      toast.error("âŒ Erro: Oficina nÃ£o identificada");
      return;
    }
    
    if (profiles.length === 0) {
      toast.error("âŒ Nenhum perfil disponÃ­vel. Configure perfis antes.", { duration: 6000 });
      return;
    }
    
    if (!formData.profile_id) {
      toast.error("âš ï¸ Selecione um Perfil de Acesso");
      return;
    }
  ...
  const copyLink = () => {
    if (createdUser?.invite_link) {
      ...
      toast.success("âœ… Link copiado para a Ã¡rea de transferÃªncia!");
    }
  };
  ...
  const getInviteLinkForEmployee = (employeeId) => {
    ...
    // Link com profile_id (sem workshop_id - serÃ¡ obtido via perfil)
      ...
      <div className="max-w-5xl mx-auto">
          ...
          <Card className="mb-6 border-2 border-green-400 bg-gradient-to-br from-green-50 to-emerald-50 shadow-lg animate-slide-up">
            <CardHeader className="pb-2 bg-gradient-to-r from-green-500 to-emerald-500 text-white rounded-t-lg">
              <CardTitle className="flex items-center gap-2 text-white">
                ...
                âœ… Link de Acesso Gerado!
              </CardTitle>
            </CardHeader>
            <CardContent className="space-y-4 pt-6">
              {/* SEÃ‡ÃƒO PRINCIPAL - Link para WhatsApp */}
              <div className="bg-white rounded-xl p-6 border-2 border-green-300 shadow-md">
                ...
                <div className="bg-gradient-to-b from-blue-50 to-blue-100 p-6 rounded-lg border-2 border-blue-400 mb-4">
                  <p className="text-xs text-blue-700 mb-2 font-semibold">ðŸ”— Link de Acesso (7 dias):</p>
                ...
                <p className="text-xs text-green-700 mt-4 text-center italic">
                  ðŸ’¡ Cole o link ou a mensagem formatada direto no WhatsApp do colaborador
                </p>
              </div>

              {/* InformaÃ§Ãµes Adicionais */}
              <div className="bg-gray-50 rounded-lg p-4 border border-gray-200">
                <p className="text-xs font-semibold text-gray-700 mb-3">ðŸ“‹ Dados do Colaborador:</p>
                <div className="space-y-2 text-sm text-gray-600 font-mono">
                  <p><strong>ðŸ“§ Email:</strong> {createdUser.email}</p>
                  <p><strong>ðŸ”‘ Senha temporÃ¡ria:</strong> {createdUser.temporary_password}</p>
                  <p><strong>â° Validade do link:</strong> 7 dias</p>
                </div>
              </div>
        ...
        {workshop && (
          <EmailPreview 
            ...
            workshopName={workshop.name}
            inviteLink={createdUser?.invite_link || formData.email ? `https://[seu-domÃ­nio]/PrimeiroAcesso?token=[token-serÃ¡-gerado]` : "https://..."}
              ...
              <div className="mt-3 bg-blue-50 border border-blue-200 rounded-lg p-3">
                <div className="flex gap-2 items-start">
                  ...
                  <p className="text-xs text-blue-800">
                    <strong>ðŸ”‘ Senha temporÃ¡ria:</strong> Sistema gera senha "Oficina@2025" - colaborador troca no primeiro login
                  </p>
                </div>
              </div>
            </CardHeader>
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div>
                  ...
                  <Input
                    ...
                    placeholder="Ex: MecÃ¢nico, Vendedor"
                    className="bg-gray-50 focus:bg-white transition-colors"
                  />
                </div>
                ...
                <div>
                   <Label htmlFor="job_role">FunÃ§Ã£o do Sistema *</Label>
                ...
                <div>
                   <Label htmlFor="role">NÃ­vel de Acesso *</Label>
                   <Select value={formData.role} onValueChange={(value) => setFormData({ ...formData, role: value })}>
                     <SelectTrigger className="bg-gray-50 focus:bg-white transition-colors">
                       <SelectValue placeholder="Selecione o nÃ­vel" />
                     </SelectTrigger>
                     <SelectContent>
                       <SelectItem value="user">UsuÃ¡rio (Colaborador)</SelectItem>
                ...
                <div className="bg-blue-50 border-2 border-blue-300 rounded-lg p-4">
                  <Label htmlFor="profile_id" className="text-blue-900 font-bold flex items-center gap-2">
                    ...
                    Perfil de Acesso * (Define PermissÃµes)
                  </Label>
                  ...
                  {isLoadingProfiles ? (
                    ...
                    <div className="mt-2 bg-red-50 border border-red-200 rounded p-3">
                      <p className="text-sm text-red-800 font-medium mb-2">
                        âŒ Nenhum perfil de acesso disponÃ­vel
                      </p>
                    ...
                    <>
                      ...
                      <p className="text-xs text-blue-700 mt-2">
                        âœ… <strong>Acesso liberado automaticamente</strong> - O colaborador poderÃ¡ fazer login assim que completar o cadastro
                      </p>
                    </>
                  )}
                </div>
            ...
            <CardContent className="p-0">
              {isLoadingEmployees ? (
                ...
                <div className="text-center py-12 px-4 text-gray-500">
                  ...
                  <p className="text-sm mt-1">Crie o primeiro acesso usando o formulÃ¡rio.</p>
                </div>
              ) : (

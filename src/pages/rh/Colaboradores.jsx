...
export default function Colaboradores() {
  ...
  // Hook de atribuiÃ§Ã£o automÃ¡tica de perfis
  ...
  const { data: workshop } = useQuery({
    ...
    queryFn: async () => {
        ...
        // Fluxo normal - buscar oficina do usuÃ¡rio
  ...
  const quickTips = [
    "Use a IA para sugestÃµes personalizadas de treinamento para cada colaborador",
    "O CDC (ConexÃ£o e DiagnÃ³stico) aumenta o salÃ¡rio emocional da sua equipe",
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-8">
          <div>
            ...
            <p className="text-gray-600">Gerencie sua equipe com inteligÃªncia artificial</p>
          </div>
          <Button
            onClick={async () => {
              const allowed = await checkPermission('employees', 'create', {
                onDenied: () => toast.error('VocÃª nÃ£o tem permissÃ£o para criar colaboradores')
              });
          ...
          {/* Button NotificaÃ§Ãµes removido e movido para Admin */}
        </div>
          ...
          <Card className="shadow-lg">
            <CardContent className="p-12 text-center">
              ...
              <p className="text-gray-600 mb-6">
                Cadastre colaboradores para comeÃ§ar
              </p>
              <Button
            ...
            <table className="w-full" id="lista-colaboradores">
              <thead className="bg-gray-100 border-b-2 border-gray-300">
                <tr>
                  ...
                  <th className="px-4 py-3 text-right text-xs font-semibold text-gray-700">ProduÃ§Ã£o</th>
                  <th className="px-4 py-3 text-center text-xs font-semibold text-gray-700">Produtividade</th>
                  <th className="px-4 py-3 text-center text-xs font-semibold text-gray-700">AÃ§Ãµes</th>
                </tr>
              </thead>
                      ...
                      <td className="px-4 py-3 text-center">
                        <div className="flex flex-wrap gap-1 justify-center">
                          <Button
                            onClick={async () => {
                              const allowed = await checkPermission('employees', 'read', {
                                onDenied: () => toast.error('Sem permissÃ£o para ver detalhes')
                              });
                          ...
                          <Button
                            onClick={async () => {
                              const allowed = await checkPermission('employees', 'read', {
                                onDenied: () => toast.error('Sem permissÃ£o para ver perfil')
                              });
                          ...
                          <Button
                            onClick={async () => {
                              const allowed = await checkPermission('employees', 'create', {
                                onDenied: () => toast.error('Sem permissÃ£o para convidar')
                              });
                          ...
                          <Button
                            onClick={async () => {
                              const allowed = await checkPermission('employees', 'update', {
                                onDenied: () => toast.error('Sem permissÃ£o para usar IA')
                              });
                              if (allowed) setSelectedEmployee(employee);
                            }}
                            size="sm"
                            className="bg-purple-600 hover:bg-purple-700"
                            title="SugestÃµes de IA"
                            disabled={checking}
                          >
                          ...
                          <Button
                            onClick={async () => {
                              const allowed = await checkPermission('employees', 'update', {
                                onDenied: () => toast.error('Sem permissÃ£o para alterar status')
                              });
                              if (!allowed) return;
                          ...
                          <Button
                            onClick={async () => {
                              const allowed = await checkPermission('employees', 'delete', {
                                onDenied: () => toast.error('Sem permissÃ£o para excluir')
                              });
                              if (!allowed) return;
                              
                              if (confirm(`EXCLUIR ${employee.full_name}? AÃ§Ã£o irreversÃ­vel!`)) {
                                try {
                                  ...
                                  if (!validation.data?.can_delete) {
                                    toast.error('Backend negou a exclusÃ£o');
                                    return;
                                  }
                                  
                                  await base44.entities.Employee.delete(employee.id);
                                  queryClient.invalidateQueries({ queryKey: ['employees'] });
                                  toast.success('Colaborador excluÃ­do');
                                } catch (error) {

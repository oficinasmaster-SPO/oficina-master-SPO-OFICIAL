...
export default function AutoavaliacaoEmpresarial() {
  ...
  const criteria = assessmentCriteria.empresarial;
  ...
  const loadData = async () => {
    try {
      ...
      base44.auth.redirectToLogin(createPageUrl("AutoavaliacaoEmpresarial"));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const aiPrompt = `
Analise a autoavaliação Empresarial Geral de uma oficina automotiva com as seguintes notas (0-10):
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex justify-center">
            <Button
              type="submit"
              disabled={submitting}
              className="bg-pink-600 hover:bg-pink-700 text-lg px-12 py-6"
            >
              {submitting ? (
                <>

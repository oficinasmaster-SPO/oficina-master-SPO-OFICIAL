...
export default function TestUsuarios() {
  ...
  const handleFixIssues = async () => {
    ...
    setLoading(true);
    try {
      // Chamar funÃ§Ã£o backend que corrige tudo
      const result = await base44.functions.invoke('linkUserToEmployee', {});
      
      if (result.data.success) {
        alert('âœ… Problemas corrigidos com sucesso!');
        await loadDebugData();
      } else {
        alert('âŒ Erro: ' + result.data.error);
      }
    } catch (error) {
      console.error('Erro ao corrigir:', error);
      alert('âŒ Erro ao corrigir: ' + error.message);
    } finally {
      setLoading(false);
    }
  };
  ...
  const loadDebugData = async () => {
    ...
    try {
      ...
      // Se nÃ£o encontrou por user_id, buscar por email
      if (!employee) {
        try {
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-3xl font-bold">Debug de PermissÃµes do UsuÃ¡rio</h1>
      ...
      <Card>
        ...
        <CardContent>
          {data?.user ? (
            ...
            <p className="text-red-600">UsuÃ¡rio nÃ£o autenticado</p>
          )}
        </CardContent>
      </Card>
        ...
        <CardContent>
          {data?.employee ? (
            <div className="space-y-2">
              ...
              <div><strong>User ID:</strong> {data.employee.user_id || <Badge variant="destructive">NÃƒO VINCULADO</Badge>}</div>
            ...
            <p className="text-red-600">Employee nÃ£o encontrado para este email</p>
          )}
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent>
          {data?.userProfile ? (
            ...
            <p className="text-red-600">UserProfile nÃ£o encontrado</p>
          )}
        </CardContent>
      </Card>
      ...
      <Card className="border-2 border-blue-500">
        <CardHeader>
          <CardTitle>ðŸ” DiagnÃ³stico Final</CardTitle>
        </CardHeader>
        <CardContent className="space-y-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <span>Profile ID atribuÃ­do ao Employee</span>
          </div>
          <div className="flex items-center gap-2">
            {data?.userProfile ? <CheckCircle2 className="w-5 h-5 text-green-600" /> : <XCircle className="w-5 h-5 text-red-600" />}
          ...
          <div className="mt-6 p-4 bg-yellow-50 border border-yellow-200 rounded">
            <h4 className="font-semibold text-yellow-800 mb-2">âŒ Problemas Identificados:</h4>
            <ul className="text-sm text-yellow-700 space-y-1">
              {!data?.employee?.user_id && <li>â€¢ Employee SEM user_id - nÃ£o consegue buscar perfil</li>}
              {!data?.employee?.profile_id && <li>â€¢ Employee SEM profile_id - nenhum perfil atribuÃ­do</li>}
              {!data?.userProfile && data?.employee?.profile_id && <li>â€¢ UserProfile nÃ£o existe no banco</li>}
              {data?.userProfile && (!data?.customRoles || data.customRoles.length === 0) && <li>â€¢ UserProfile sem CustomRoles vinculadas</li>}
              {data?.customRoles?.length > 0 && !data.customRoles.some(r => r.system_roles?.length > 0) && <li>â€¢ CustomRoles sem system_roles configuradas</li>}
            </ul>
          </div>
        </CardContent>
      </Card>

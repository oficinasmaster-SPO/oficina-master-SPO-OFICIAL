...
export default function Tarefas() {
  ...
  const { data: employees = [] } = useQuery({
    ...
    queryFn: async () => {
      if (!workshop) return [];
      try {
        // Filter by workshop_id to prevent cross-company data leak
      ...
      if (task.assigned_to && task.assigned_to.length > 0) {
        for (const userId of task.assigned_to) {
          await base44.entities.Notification.create({
            ...
            message: `VocÃª foi atribuÃ­do Ã  tarefa: ${task.title}`,
            is_read: false
          });
        }
      }
  ...
  const handleStatusChange = async (id, newStatus) => {
    ...
    if (task?.created_by && newStatus === 'concluida') {
      await base44.entities.Notification.create({
        ...
        message: `A tarefa "${task.title}" foi marcada como concluÃ­da`,
        is_read: false
      });
    }
  };
  ...
  const tourSteps = [
    {
      ...
      description: "Clique aqui para criar uma nova tarefa. VocÃª pode adicionar tÃ­tulo, descriÃ§Ã£o, prioridade, data de vencimento e atribuir pessoas.",
      position: "bottom"
    },
    {
      target: "view-mode-buttons",
      title: "VisualizaÃ§Ãµes",
      description: "Alterne entre visualizaÃ§Ã£o em lista ou Kanban drag-and-drop para organizar suas tarefas da forma que preferir.",
      position: "bottom"
    },
    {
      target: "stats-section",
      title: "EstatÃ­sticas",
      description: "Acompanhe em tempo real o total de tarefas, pendentes, em andamento, concluÃ­das e atrasadas.",
      position: "bottom"
    },
    {
      target: "filters-section",
      title: "Filtros AvanÃ§ados",
      description: "Use os filtros para encontrar tarefas especÃ­ficas por status, prioridade, responsÃ¡vel ou prazo.",
      position: "top"
    },
    {
      target: "task-card",
      title: "CartÃ£o de Tarefa",
      description: "Cada tarefa mostra prioridade, status, progresso, vencimento e responsÃ¡veis. VocÃª pode editar, excluir ou mudar o status rapidamente.",
      position: "top"
    }
  ];
  ...
  const helpContent = {
    ...
    description: "Este mÃ³dulo permite criar, organizar e acompanhar tarefas da sua equipe com visualizaÃ§Ã£o Kanban, dependÃªncias entre tarefas e lembretes configurÃ¡veis.",
    faqs: [
      {
        question: "Como criar uma nova tarefa?",
        answer: "Clique no botÃ£o 'Nova Tarefa', preencha as informaÃ§Ãµes e atribua responsÃ¡veis. VocÃª pode configurar lembretes e dependÃªncias."
      },
      {
        question: "O que Ã© a visualizaÃ§Ã£o Kanban?",
        answer: "Kanban permite arrastar tarefas entre colunas de status (Pendente, Em Andamento, ConcluÃ­da) para atualizaÃ§Ã£o rÃ¡pida."
      },
      {
        question: "Como funcionam as dependÃªncias?",
        answer: "DependÃªncias impedem que uma tarefa seja concluÃ­da antes que outras tarefas vinculadas estejam finalizadas."
      },
      {
        question: "Como configurar lembretes?",
        answer: "Ao criar/editar uma tarefa, configure lembretes por e-mail ou notificaÃ§Ã£o no app antes do vencimento."
      }
    ]
  };
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div id="stats-section" className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-6">
          ...
          <div className="bg-green-50 rounded-lg p-4 shadow-sm border-2 border-green-200">
            <p className="text-sm text-green-800">ConcluÃ­das</p>

...
export default function ContasPagar() {
  ...
  const [contaParaEstornar, setContaParaEstornar] = useState(null);
  ...
  const totalAberto  = contas?.filter(c => c.status === "aberto").reduce((s, c) => s + (c.valor_aberto || 0), 0) || 0;
  const totalVencido = contas?.filter(c => c.status === "vencido").reduce((s, c) => s + (c.valor_aberto || 0), 0) || 0;
  ...
  return (
    <div className="container mx-auto p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <Card>
          <CardHeader className="pb-2"><CardTitle className="text-sm font-medium">Total Aberto</CardTitle></CardHeader>
          <CardContent><div className="text-2xl font-bold">R$ {totalAberto.toFixed(2)}</div></CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-2"><CardTitle className="text-sm font-medium">Total Vencido</CardTitle></CardHeader>
          <CardContent><div className="text-2xl font-bold text-red-600">R$ {totalVencido.toFixed(2)}</div></CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-2"><CardTitle className="text-sm font-medium">Contas</CardTitle></CardHeader>
          <CardContent><div className="text-2xl font-bold">{contas?.length || 0}</div></CardContent>
        </Card>
      </div>
                    ...
                    <TableRow>
                      ...
                      <TableCell>
                        <Button
                          ...
                          onClick={() =>
                            setContaHistoricoAberta(contaHistoricoAberta === conta.id ? null : conta.id)
                          }
                        >
      ...
      {contaParaPagar && (
        <ModalRegistrarPagamentoConta
          ...
          onSuccess={() => { setContaParaPagar(null); refetch(); }}
        />
      )}
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              <strong>{contaParaDeletar?.fornecedor_nome}</strong> no valor de{" "}
              <strong>R$ {contaParaDeletar?.valor_original?.toFixed(2)}</strong>?
              <br />

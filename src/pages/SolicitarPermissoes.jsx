  ...
  if (!myEmployee) {
    return (
      <Alert className="max-w-2xl mx-auto mt-8">
        ...
        <AlertDescription>
          Seu cadastro de colaborador não foi encontrado. Entre em contato com o administrador.
        </AlertDescription>
      </Alert>
    );
  }
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Solicitar Permissões</h1>
          <p className="text-gray-600 mt-1">
            Portal de autoatendimento para solicitação de acessos
          </p>
        </div>
      </div>

      {/* Minhas Permissões Atuais */}
      <Card>
        <CardHeader>
          <CardTitle>Minhas Permissões Atuais</CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          <div>
            <p className="text-sm font-medium text-gray-700 mb-2">Perfil Atribuído:</p>
            {currentProfile ? (
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs text-gray-600">
                  {currentProfile.roles?.length || 0} permissões
                </span>
              </div>
            ) : (
              <p className="text-sm text-gray-500">Nenhum perfil atribuído</p>
            )}
          </div>
          ...
          <div className="pt-4 border-t">
            <p className="text-xs text-gray-500">
              Para solicitar alterações nas suas permissões, clique em uma das opções abaixo.
            </p>
          </div>
        </CardContent>
      </Card>
        ...
        <Card className="hover:shadow-lg transition-shadow cursor-pointer border-2 border-transparent hover:border-blue-500"
          ...
          <CardContent className="pt-6">
            <div className="text-center space-y-3">
              ...
              <h3 className="font-semibold text-gray-900">Solicitar Mudança de Perfil</h3>
              <p className="text-sm text-gray-600">
                Solicite a alteração do seu perfil de acesso
              </p>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="hover:shadow-lg transition-shadow cursor-pointer border-2 border-transparent hover:border-purple-500"
          ...
          <CardContent className="pt-6">
            <div className="text-center space-y-3">
              ...
              <p className="text-sm text-gray-600">
                Solicite permissões específicas adicionais
              </p>
            </div>
          </CardContent>
        </Card>
                    ...
                    <div className="flex-1">
                      ...
                      <div className="text-xs text-gray-600 space-y-1">
                        ...
                        {request.change_type === 'profile_change' && (
                          <p>
                            <strong>Mudança:</strong> {request.current_profile_name || 'Sem perfil'} → {request.requested_profile_name || 'A definir'}
                          </p>
                        )}
                        ...
                        {request.status === 'rejeitado' && (
                          <p className="text-red-700">
                            <strong>Motivo da rejeição:</strong> {request.rejection_reason || 'Não informado'}
                          </p>
                        )}
                      </div>
                    </div>

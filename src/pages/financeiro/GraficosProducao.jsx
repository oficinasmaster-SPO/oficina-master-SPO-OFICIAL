...
// FunÃ§Ã£o para calcular dias Ãºteis do mÃªs
const calcularDiasUteis = (mesAno) => {
  ...
  for (let dia = primeiroDia.getDate(); dia <= ultimoDia.getDate(); dia++) {
    ...
    // Contar apenas segunda a sexta (1-5), excluindo sÃ¡bado (6) e domingo (0)
...
export default function GraficosProducao() {
  ...
  const { data: records = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      // ForÃ§a reload do workshop para pegar melhor mÃªs atualizado
  ...
  // Calcular dias Ãºteis do mÃªs filtrado e metas diÃ¡rias
  ...
  // Calcular metas DIÃRIAS dividindo pelo nÃºmero de dias Ãºteis
  const metaDiaria = {
  ...
  // Calcular dias decorridos do mÃªs atÃ© hoje
  const hoje = new Date();
  ...
  if (anoFiltro === anoAtual && mesFiltro === mesAtual) {
    // Se Ã© o mÃªs atual, contar apenas dias Ãºteis atÃ© hoje
    ...
    // Se Ã© mÃªs passado, usar todos os dias Ãºteis do mÃªs
    diasDecorridos = diasUteis;
  }

  // Calcular meta acumulada atÃ© o dia
  const metaAcumulada = {
        ...
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-blue-300/70 mt-1">AnÃ¡lise visual do desempenho da oficina</p>
            </div>
          </div>

          <div className="flex items-center gap-3">
            <div>
              <Label className="text-xs text-blue-300/70 mb-1 block">Filtrar MÃªs</Label>
              <Input
                type="month"
                value={filterMonth}
        ...
        {isLoading ? (
          ...
          <Card className="bg-slate-800/50 border-blue-500/30">
            <CardContent className="p-12 text-center">
              ...
              <p className="text-blue-300/70">Nenhum registro encontrado para este perÃ­odo</p>
            </CardContent>
          </Card>
        ) : (
          <Tabs defaultValue="faturamento" className="w-full">
                ...
                <CardContent>
                  <ResponsiveContainer width="100%" height={350}>
                    <LineChart data={chartData}>
                      ...
                      <Line type="monotone" dataKey="meta_diaria_pave" stroke="#ec4899" strokeWidth={2} strokeDasharray="5 5" name="Meta DiÃ¡ria" dot={{ fill: '#ec4899', r: 3 }} />
                    </LineChart>
                  </ResponsiveContainer>
                </CardContent>
              </Card>
                ...
                <CardContent>
                  <ResponsiveContainer width="100%" height={350}>
                    <LineChart data={chartData}>
                      ...
                      <Line type="monotone" dataKey="meta_diaria_kit_master" stroke="#f97316" strokeWidth={2} strokeDasharray="5 5" name="Meta DiÃ¡ria" dot={{ fill: '#f97316', r: 3 }} />
                    </LineChart>
                  </ResponsiveContainer>
                </CardContent>
              </Card>
                ...
                <CardContent>
                  <ResponsiveContainer width="100%" height={350}>
                    <LineChart data={chartData}>
                      ...
                      <Line type="monotone" dataKey="meta_diaria_gps" stroke="#f59e0b" strokeWidth={2} strokeDasharray="5 5" name="Meta DiÃ¡ria" dot={{ fill: '#f59e0b', r: 3 }} />
                    </LineChart>
                  </ResponsiveContainer>
                </CardContent>
              </Card>
                ...
                <CardContent>
                  <ResponsiveContainer width="100%" height={350}>
                    <LineChart data={chartData}>
                      ...
                      <Line type="monotone" dataKey="meta_diaria_vendas_base" stroke="#3b82f6" strokeWidth={2} strokeDasharray="5 5" name="Meta DiÃ¡ria Base" dot={{ fill: '#3b82f6', r: 3 }} />
                      <Line type="monotone" dataKey="vendas_mkt" stroke="#ec4899" strokeWidth={2} name="Vendas Marketing R$" dot={{ fill: '#ec4899', r: 3 }} />
                      <Line type="monotone" dataKey="meta_diaria_vendas_mkt" stroke="#f59e0b" strokeWidth={2} strokeDasharray="5 5" name="Meta DiÃ¡ria Mkt" dot={{ fill: '#f59e0b', r: 3 }} />
                    </LineChart>
                  </ResponsiveContainer>
                </CardContent>
              </Card>
            ...
            <TabsContent value="agendamentos" className="space-y-6">
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <GapFunnelIndicator
                  metricName="Agendamentos (SDR â†’ Closer)"
                   ...
                   <CardTitle className="text-transparent bg-clip-text bg-gradient-to-r from-red-400 to-rose-400">Investimento vs Faturamento</CardTitle>
                    <StatusMetaBadge
                      ...
                      onClick={() => setFeedbackModal({
                        ...
                        metricName: "Faturamento Lead TrÃ¡fego",
                ...
                <CardContent>
                  <ResponsiveContainer width="100%" height={350}>
                    <LineChart data={chartData}>
                      ...
                      <Tooltip content={<CustomTooltip />} />
                      <Legend wrapperStyle={{ color: '#60a5fa' }} />
                      <Line type="monotone" dataKey="investido_trafego" stroke="#ef4444" strokeWidth={2} name="Investido TrÃ¡fego R$" dot={{ fill: '#ef4444', r: 3 }} />
                      <Line type="monotone" dataKey="meta_diaria_investido_trafego" stroke="#f87171" strokeWidth={2} strokeDasharray="5 5" name="Meta DiÃ¡ria Invest." dot={{ fill: '#f87171', r: 2 }} />
                      <Line type="monotone" dataKey="faturado_lead" stroke="#10b981" strokeWidth={2} name="Faturado Lead R$" dot={{ fill: '#10b981', r: 3 }} />
                      <Line type="monotone" dataKey="meta_diaria_faturado_lead" stroke="#34d399" strokeWidth={2} strokeDasharray="5 5" name="Meta DiÃ¡ria Fatur." dot={{ fill: '#34d399', r: 2 }} />
                    </LineChart>
                  </ResponsiveContainer>
                </CardContent>
              </Card>
                ...
                <CardContent>
                  <ResponsiveContainer width="100%" height={350}>
                    <LineChart data={chartData}>
                      ...
                      <Line type="monotone" dataKey="meta_diaria_custo_venda" stroke="#f59e0b" strokeWidth={2} strokeDasharray="5 5" name="Meta DiÃ¡ria" dot={{ fill: '#f59e0b', r: 3 }} />
                    </LineChart>
                  </ResponsiveContainer>
                </CardContent>
              </Card>

  ...
  const validateToken = async () => {
    ...
    try {
      ...
      if (response.data.valid) {
        ...
        toast.error(response.data.error || "Link invÃ¡lido");
      }
    } catch (error) {
      console.error(error);
      ...
      <div className="min-h-screen flex items-center justify-center bg-slate-50 p-4">
        <Card className="w-full max-w-md text-center">
          <CardContent className="pt-6">
            ...
            <h2 className="text-xl font-bold text-gray-900">Link InvÃ¡lido ou Expirado</h2>
          </CardContent>
        </Card>
      </div>
    );
      ...
      <div className="min-h-screen flex items-center justify-center bg-slate-50 p-4">
        <Card className="w-full max-w-md text-center">
          <CardContent className="pt-6">
            ...
            <p className="text-gray-600">Sua autoavaliaÃ§Ã£o foi registrada.</p>
          </CardContent>
        </Card>
      </div>
    );
    ...
    <div className="min-h-screen bg-slate-50 py-8 px-4">
      <div className="max-w-3xl mx-auto">
        <div className="mb-8 text-center">
          <h1 className="text-2xl font-bold text-gray-900">AutoavaliaÃ§Ã£o Profissional</h1>
          <p className="text-gray-600">
            Colaborador: <strong>{inviteData?.candidate_name || 'VocÃª'}</strong>
          </p>
        </div>

        <div className="mb-6">
          <div className="flex justify-between text-sm text-gray-600 mb-2">
            <span>QuestÃ£o {currentQuestion + 1} de {maturityQuestions.length}</span>
        ...
        <div className="mt-8 flex justify-end">
          ...
          >
            {submitting ? <Loader2 className="animate-spin" /> : (
              currentQuestion === maturityQuestions.length - 1 ? "Finalizar" : "PrÃ³xima"
            )}
          </Button>
        </div>
      </div>

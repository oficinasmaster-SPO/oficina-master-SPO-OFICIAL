...
export default function ResponderDISC() {
  ...
  const validateToken = async () => {
    ...
    try {
      ...
      if (response.data.valid) {
        ...
        toast.error(response.data.error || "Link invÃ¡lido ou expirado");
      }
    } catch (error) {
  ...
  const updateAnswer = (questionId, profile, value) => {
    ...
    if (value !== "" && usedNumbers.includes(value)) {
      toast.error(`NÃºmero ${value} jÃ¡ foi usado neste conjunto. Escolha outro.`);
      return;
    }
  ...
  const handleSubmit = async (e) => {
    ...
    if (getFilledQuestions() < 10) {
      toast.error("Por favor, responda todas as questÃµes.");
      return;
    }
      ...
      <div className="min-h-screen flex items-center justify-center bg-slate-50 p-4">
        <Card className="w-full max-w-md text-center">
          <CardContent className="pt-6">
            ...
            <h2 className="text-xl font-bold text-gray-900 mb-2">Link InvÃ¡lido ou Expirado</h2>
            <p className="text-gray-600">Este link de avaliaÃ§Ã£o nÃ£o estÃ¡ mais disponÃ­vel.</p>
          </CardContent>
        </Card>
      </div>
    );
      ...
      <div className="min-h-screen flex items-center justify-center bg-slate-50 p-4">
        <Card className="w-full max-w-md text-center">
          <CardContent className="pt-6">
            ...
            <h2 className="text-2xl font-bold text-gray-900 mb-2">AvaliaÃ§Ã£o ConcluÃ­da!</h2>
      ...
      <div className="max-w-4xl mx-auto">
        <div className="text-center mb-8">
          ...
          <p className="text-gray-600">
            OlÃ¡, <strong>{inviteData?.candidate_name || 'Colaborador'}</strong>. Por favor, responda com sinceridade.
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <Card className="bg-amber-50 border-2 border-amber-300">
            <CardContent className="p-4 text-sm text-amber-900">
              <strong>âš ï¸ IMPORTANTE - OrdenaÃ§Ã£o Ãšnica:</strong> Para cada grupo, use os nÃºmeros de <strong>1 a 4 apenas UMA VEZ cada</strong>:
              <br />â€¢ <strong>4</strong> = CaracterÃ­stica que MAIS se identifica com vocÃª
              <br />â€¢ <strong>3</strong> = Segunda caracterÃ­stica mais identificada
              <br />â€¢ <strong>2</strong> = Terceira caracterÃ­stica
              <br />â€¢ <strong>1</strong> = CaracterÃ­stica que MENOS se identifica
              <br /><br />
              <strong>Exemplo:</strong> Se vocÃª colocar "4" na OpÃ§Ã£o A, nÃ£o pode usar "4" novamente. Cada nÃºmero sÃ³ pode ser usado uma vez por grupo.
            </CardContent>
          </Card>

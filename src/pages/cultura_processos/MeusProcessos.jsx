...
export default function MeusProcessos() {
  ...
  // Calculate metrics - contando todos os documentos acessÃ­veis
  const mapsCount = accessibleDocs.length; // Total de MAPs disponÃ­veis
  ...
  // Processos customizados (criados pela oficina, nÃ£o templates)
  ...
  const getDetailsData = (type) => {
    switch (type) {
      ...
      case "its":
        return workshopIts;
      case "frs":
        // Aqui retornarÃ­amos os FRs se tivessem entidade prÃ³pria
        return [];
  ...
  const duplicateMutation = useMutation({
    mutationFn: async (doc) => {
      if (!workshop) throw new Error("Oficina nÃ£o encontrada");
      
      const newDoc = {
        title: `${doc.title} (CÃ³pia)`,
        code: doc.code,
        revision: "1",
  ...
  const categories = [
    ...
    { id: "PÃ¡tio", label: "PÃ¡tio", icon: Truck },
          ...
          <TabsContent value={activeTab} className="mt-0">
            {isLoading ? (
              ...
              <ProcessHierarchyView
                ...
                onSelectIT={(it) => toast.info("VisualizaÃ§Ã£o de IT em desenvolvimento")}
              />
            ) : (
                    ...
                    <Card key={doc.id} className="hover:shadow-lg transition-shadow duration-300 flex flex-col">
                      <CardHeader className="pb-3">
                        ...
                        <CardDescription className="line-clamp-2 mt-2">
                         {doc.description || "Sem descriÃ§Ã£o disponÃ­vel."}
                        </CardDescription>
                      ...
                      <CardFooter className="pt-0 gap-2 flex-wrap">
                        ...
                        <Button 
                          ...
                          title="Ver HistÃ³rico de Compartilhamentos"

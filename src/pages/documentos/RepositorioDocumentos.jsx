...
export default function RepositorioDocumentos() {
  ...
  const { data: documents = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      console.log("ðŸ” [QUERY] Iniciando busca de documentos");
      console.log("ðŸ” [QUERY] Workshop:", workshop);
      console.log("ðŸ” [QUERY] User:", user?.email, user?.role);
      
      if (!workshop) {
        console.error("âŒ [QUERY] Workshop nÃ£o encontrado - nÃ£o pode buscar documentos");
        return [];
      }
      
      console.log(`ðŸ” [QUERY] Buscando documentos para workshop_id: "${workshop.id}"`);
      console.log(`ðŸ” [QUERY] Nome do workshop: "${workshop.name}"`);
      
      try {
        const docs = await base44.entities.CompanyDocument.filter({ workshop_id: workshop.id }, '-created_date');
        console.log(`âœ… [QUERY] Query retornou ${docs.length} documentos`);
        
        if (docs.length > 0) {
          console.log("ðŸ“„ [QUERY] Documentos encontrados:");
          docs.forEach((d, i) => {
            console.log(`  ${i + 1}. ID: ${d.id} | Title: ${d.title} | Workshop: ${d.workshop_id}`);
          });
        } else {
          console.warn("âš ï¸ [QUERY] Nenhum documento retornado pela query");
          console.log("âš ï¸ [QUERY] Tentando buscar TODOS os documentos (debug)...");
          const allDocs = await base44.entities.CompanyDocument.list();
          console.log(`âš ï¸ [QUERY] Total de documentos no sistema: ${allDocs.length}`);
          if (allDocs.length > 0) {
            console.log("âš ï¸ [QUERY] Workshops nos documentos existentes:");
        ...
        console.error("âŒ [QUERY] Erro ao buscar documentos:", error);
        return [];
      }
    },
  ...
  console.log(`ðŸ“Š Total: ${documents.length} | Exibindo: ${filteredDocuments.length}`);
  ...
  const categories = [
    ...
    { value: "consorcio", label: "ConsÃ³rcio" },
    { value: "rh", label: "RH / FuncionÃ¡rios" },
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50 py-8 px-4">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            <h1 className="text-3xl font-bold text-gray-900">RepositÃ³rio de Documentos</h1>
            ...
            <Table>
              <TableHeader className="bg-slate-50">
                <TableRow>
                  ...
                  <TableHead>Revisar AtÃ©</TableHead>

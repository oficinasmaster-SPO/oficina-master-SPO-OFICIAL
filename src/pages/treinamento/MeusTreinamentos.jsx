...
export default function MeusTreinamentos() {
  ...
  const loadData = async () => {
    try {
      ...
      // Verificar se Ã© dono de oficina
      ...
      // Encontrar meu registro de colaborador para verificar workshop e atribuiÃ§Ã£o
      ...
      if (isOwner) {
        // Se for dono, vÃª todos os mÃ³dulos da sua oficina + globais
        ...
        allModules = allModules.filter(mod => {
          ...
          // 2. Filtro por AtribuiÃ§Ã£o EspecÃ­fica (assigned_to_ids)
          if (mod.assigned_to_ids && mod.assigned_to_ids.length > 0) {
            return mod.assigned_to_ids.includes(myEmployee.id);
          }
          // Se assigned_to_ids estiver vazio, Ã© pÃºblico para a oficina (ou global)
          return true;
        });
      } else {
        // UsuÃ¡rio sem vÃ­nculo (nem dono, nem colaborador) -> vÃª apenas globais
      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex items-center justify-between mb-10">
          <div>
            ...
            <p className="text-slate-600 mt-1">Desenvolva suas habilidades e acompanhe sua evoluÃ§Ã£o.</p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {modules.length === 0 ? (
            <div className="col-span-full text-center py-12">
              ...
              <p className="text-slate-500 text-lg">Nenhum treinamento disponÃ­vel no momento.</p>
            </div>
          ) : (
            modules.map(module => {
                  ...
                  <div className="h-48 bg-slate-200 relative overflow-hidden">
                    ...
                    {isCompleted && (
                      <div className="absolute top-4 right-4 bg-green-500 text-white px-3 py-1 rounded-full text-xs font-bold flex items-center gap-1 shadow-lg">
                        <Trophy className="w-3 h-3" /> ConcluÃ­do
                      </div>
                    )}
                  </div>
                  ...
                  <CardContent className="p-5">
                    ...
                    <div className="space-y-2">
                      <div className="flex justify-between text-xs font-semibold text-slate-500">
                        <span>{prog.percent}% ConcluÃ­do</span>

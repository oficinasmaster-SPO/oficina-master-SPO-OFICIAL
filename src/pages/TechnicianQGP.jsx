    ...
    <div className="min-h-screen bg-gray-100 p-4 pb-24">
      <div className="max-w-md mx-auto space-y-6">
        ...
        {activeTask ? (
          <Card className="border-2 border-green-500 shadow-lg relative overflow-hidden">
            ...
            <CardHeader className="pb-2">
              <div className="flex justify-between items-start">
                <Badge className="bg-green-500 hover:bg-green-600">EM EXECUÇÃO</Badge>
              ...
              <p className="text-gray-500 text-sm">{activeTask.qgp_data?.vehicle_model} • {activeTask.qgp_data?.vehicle_plate}</p>
            </CardHeader>
            <CardContent className="space-y-4">
               ...
               <div className="flex items-center gap-2 bg-gray-50 p-3 rounded-lg border border-gray-100">
                  ...
                  <div>
                    ...
                    <p className="font-bold text-gray-900">
                      {activeTask.predicted_time_minutes ? `${activeTask.predicted_time_minutes / 60}h` : <span className="text-red-500 text-xs">Não definido</span>}
                    </p>
                  </div>
               ...
               {activeTask.qgp_data?.os_file_url && (
                 <Button variant="outline" className="w-full justify-between group" onClick={() => { setSelectedTask(activeTask); setViewOsDialog(true); }}>
                   <span className="flex items-center gap-2"><FileText className="w-4 h-4 text-gray-500" /> Ver Ordem de Serviço</span>
          ...
          <div className="bg-white p-6 rounded-xl border border-dashed border-gray-300 text-center text-gray-500">
            <p>Você não tem serviços em execução.</p>
            <p className="text-sm mt-1">Inicie um serviço da lista abaixo.</p>
          </div>
        )}

        {/* Task List */}
        <div className="space-y-3">
          <h3 className="font-semibold text-gray-700 pl-1">Próximos Serviços</h3>
              ...
              <CardContent className="p-4">
                <div className="flex justify-between items-start mb-2">
                   <div>
                     ...
                     <p className="text-xs text-gray-500">{task.qgp_data?.vehicle_model} • {task.qgp_data?.vehicle_plate}</p>
                   </div>
        ...
        <DialogContent>
          ...
          <div className="space-y-4 py-2">
            <div className="grid grid-cols-2 gap-2">
              {['Almoço', 'Peças', 'Aprovação', 'Fim de Expediente', 'Outros'].map(reason => (
                <Button 
                  key={reason} 
        ...
        <DialogContent>
          ...
          <div className="py-4">
             ...
             <Input 
               ...
               className="text-lg"
             />
             <p className="text-xs text-gray-500 mt-2">Insira o tempo total estimado para conclusão do serviço.</p>
          </div>
          <DialogFooter>
             <Button onClick={handleSetTime}>Salvar Previsão</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={viewOsDialog} onOpenChange={setViewOsDialog}>
        <DialogContent className="max-w-3xl h-[80vh]">
          <DialogHeader>
            <DialogTitle>Ordem de Serviço #{selectedTask?.qgp_data?.os_number}</DialogTitle>
          </DialogHeader>
          <div className="flex-1 bg-gray-100 rounded border h-full overflow-hidden">
            {selectedTask?.qgp_data?.os_file_url ? (
              ...
              <div className="flex items-center justify-center h-full text-gray-400">Arquivo indisponível</div>
            )}
          </div>
        </DialogContent>
      </Dialog>

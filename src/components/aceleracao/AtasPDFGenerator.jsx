...
export const generateAtaPDF = (rawAta, workshop) => {
  ...
  const checklistBlocos = (ata.checklist_respostas || []).filter(b => b.incluir_na_ata !== false);
  if (checklistBlocos.length > 0) {
    checkPageBreak(30);
    doc.setFontSize(13);
    doc.setFont(undefined, 'bold');
    doc.setTextColor(0, 102, 180);
    doc.text('13. CHECKLIST DE DIAGNOSTICO (Respostas do Atendimento)', margin, y);
    doc.setTextColor(0, 0, 0);
    y += 2;
    doc.setLineWidth(0.5);
    ...
    checklistBlocos.forEach(bloco => {
      ...
      doc.rect(margin, y - 4, contentWidth, 10, 'FD');
      doc.text(safeText(bloco.template_nome), margin + 2, y + 1);
      y += 10;

      bloco.perguntas.forEach((p, pIdx) => {
        checkPageBreak(24);

        // Pergunta
        doc.setFontSize(10);
        doc.setFont(undefined, 'bold');
        const pergLines = doc.splitTextToSize(`${pIdx + 1}. ${safeText(p.pergunta_texto)}`, contentWidth - 4);
        pergLines.forEach(line => {
          checkPageBreak(6);
          doc.text(line, margin + 2, y);
          y += 5;
        });
        ...
        if (p.resposta_atual) {
          ...
          doc.setFont(undefined, 'bold');
          doc.setTextColor(80, 80, 80);
          doc.text('Situacao Atual:', margin + 4, y);
          doc.setFont(undefined, 'normal');
          y += 4;
          const atualLines = doc.splitTextToSize(safeText(p.resposta_atual), contentWidth - 8);
          atualLines.forEach(line => {
            checkPageBreak(5);
        ...
        if (p.resposta_meta) {
          doc.setFontSize(9);
          doc.setFont(undefined, 'bold');
          doc.text('Objetivo (Meta):', margin + 4, y);
          doc.setFont(undefined, 'normal');
          y += 4;
          const metaLines = doc.splitTextToSize(safeText(p.resposta_meta), contentWidth - 8);
        ...
        // Atingimento
        if (p.atingimento_descritivo || p.pct_atingimento) {
          doc.setFontSize(9);
          doc.setFont(undefined, 'bold');
          doc.setTextColor(22, 101, 52);
          const atingText = [p.atingimento_descritivo && `Atingimento: ${safeText(p.atingimento_descritivo)}`, p.pct_atingimento && `(${p.pct_atingimento}%)`].filter(Boolean).join(' ');
          doc.text(atingText, margin + 4, y);
          doc.setTextColor(80, 80, 80);
          y += 4;
        }

...
import { Play, CheckCircle, UserRound } from "lucide-react";
...
const status = { aberta: "Aberta", em_execucao: "Em execução", bloqueada: "Bloqueada", concluida: "Concluída" };
const origin = { manual: "Manual", pedido: "Pedido", reuniao: "Reunião", entrega: "Entrega", material: "Material" };
const sideTone = (t) => t.status === "concluida" ? "border-l-green-500" : t.prioridade === "critica" ? "border-l-red-500" : t.prazo && new Date(t.prazo) < new Date() ? "border-l-amber-500" : t.status === "em_execucao" ? "border-l-blue-500" : "border-l-gray-300";
  ...
  return (
    <article onClick={() => onView(tarefa)} className={`group cursor-pointer rounded-2xl border border-l-4 border-gray-200 bg-white p-5 shadow-sm transition-all hover:-translate-y-0.5 hover:border-blue-200 hover:shadow-md ${sideTone(tarefa)}`}>
      <div className="grid gap-5 lg:grid-cols-[minmax(0,7fr)_minmax(230px,3fr)]">
        <div className="min-w-0"><h3 className="truncate text-base font-semibold text-gray-950">{tarefa.titulo}</h3><p className="mt-2 text-sm font-medium text-gray-700">{tarefa.cliente_nome || "Cliente não informado"}</p><div className="mt-2 flex flex-wrap gap-x-5 gap-y-1 text-xs text-gray-500"><span className="flex items-center gap-1.5"><UserRound className="h-3.5 w-3.5" />{tarefa.consultor_nome || "Sem consultor"}</span><span>Responsável: {tarefa.atribuido_para_nome || tarefa.responsavel_nome || "—"}</span><span>Categoria: {tarefa.categoria || tarefa.impacto || "—"}</span></div>{tarefa.origem && <Badge variant="outline" className="mt-3 border-gray-200 bg-gray-50 text-[10px] font-normal text-gray-500">{origin[tarefa.origem] || tarefa.origem}</Badge>}</div>
        <div className="border-t border-gray-100 pt-4 lg:border-l lg:border-t-0 lg:pl-5 lg:pt-0"><div className="grid grid-cols-2 gap-4"><SmartDueDate date={tarefa.prazo} completed={tarefa.status === "concluida"} /><div className="flex flex-col items-end gap-2"><Badge variant="outline" className={`text-[10px] ${priorityCfg[1]}`}>{priorityCfg[0]}</Badge><Badge variant="outline" className="border-slate-200 bg-slate-100 text-[10px] text-slate-700">{status[tarefa.status] || tarefa.status}</Badge></div></div>

...
import {
  ...
  MessageSquare, Send, Loader2, FileText, CheckCheck, FileWarning
} from "lucide-react";
...
export default function IniciarAtendimentoModal({ followUp: followUpInicial, cliente, onClose, onSaved, fusConcatenados = [], proximoFU = null, onProximoFollowUp, filaReminders = [], onNavegar, openClientSelectorOnMount = false }) {
  ...
  // ── Função central de troca de follow-up (sem fechar o modal) ──
   const trocarFollowUp = useCallback((novoFU) => {
     // Liberar lock do follow-up atual antes de trocar
  ...
  // Lock — verifica se outro consultor já está atendendo este follow-up
  useEffect(() => {
    ...
    (async () => {
      ...
      } catch { /* não bloquear usuário em caso de erro */ }
    })();
    return () => { cancelled = true; };
  }, [followUp?.id]);

  // Liberar lock ao desmontar o modal
  useEffect(() => {
    return () => {
  ...
  // Posicionado APÓS handleClose para evitar ReferenceError (const não tem hoisting)
  ...
  return ReactDOM.createPortal(
    <>
      <div className="p-0 flex flex-col overflow-hidden relative bg-white animate-in fade-in duration-150" style={{ position: "fixed", inset: 0, zIndex: 9999 }}>
        ...
        {/* LOCK — follow-up sendo atendido por outro consultor */}
        {lockBloqueado && (
        ...
        {/* HEADER - FIXO */}
         <div className="text-white px-6 py-4 flex items-center justify-between border-b border-gray-800 flex-shrink-0" style={{ background: 'linear-gradient(to right, #171e2b, #111827)' }}>
           <div className="flex items-center gap-4 flex-1">
             ...
             >
               ...
               <div className="text-left">
                 ...
                 {(followUp?.origin_type === 'suporte' || followUp?.origin_type === 'suporte_checkin') ? (
                   ...
                   <span className="inline-flex items-center text-[10px] font-bold text-purple-300 bg-purple-500/15 border border-purple-500/30 rounded-full px-2 py-0.5">Follow-up {followUp?.sequence_number}/4</span>
                 )}
               </div>
             ...
             <Badge className="bg-gradient-to-r from-red-600 to-red-700 text-white border-0 shadow-lg flex items-center gap-2 px-3 py-1.5 flex-shrink-0">
               <span className="atendimento-dot w-2 h-2 rounded-full bg-green-400 flex-shrink-0" />
            ...
            <button
              ...
              className="w-9 h-9 rounded-lg flex items-center justify-center text-lg font-bold text-gray-300 hover:text-white hover:bg-gray-700 hover:scale-110 disabled:opacity-25 disabled:cursor-not-allowed disabled:hover:scale-100 transition-all"
            >
              ◀
            </button>
      ...
      <DocumentFormDialog
        ...
        onSuccess={(created) => {
          ...
          if (created?.attachments?.length > 0) {
            setUploadedDocs(prev => [...prev, ...created.attachments.map(a => ({ name: a.file_name, size: a.file_size, type: a.file_type, url: a.file_url }))]);
          } else if (created?.file_url) {
            setUploadedDocs(prev => [...prev, { name: created.title || 'documento', url: created.file_url, type: created.file_type, size: created.file_size }]);
          }
          setPendingFile(null);

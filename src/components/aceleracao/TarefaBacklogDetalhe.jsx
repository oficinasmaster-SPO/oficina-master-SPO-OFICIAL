...
import { ArrowLeft, User, Clock, AlertCircle, FileText, CheckCircle, ExternalLink, Play, Lock, Activity } from "lucide-react";
...
export default function TarefaBacklogDetalhe({ tarefa, user, onVoltar, onEditar, onConcluir }) {
  ...
  // H6: Buscar nomes de usuários em uma única query (batch em vez de 3 separadas)
  useEffect(() => {
  ...
  // H6: Todas as mutations invalidam ['tarefas-backlog'] para atualizar a lista pai
  ...
  // H3: Salvar notas de execução sem mudar status (anexos removidos — usar TaskComment)
    ...
    <div className="space-y-6">
      ...
      <section className="rounded-2xl border border-gray-200 bg-white p-4 sm:p-6">
        ...
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-5">
          <div><p className="text-xs text-gray-500">Status</p><div className="mt-1"><StatusBadge entity="tarefa" status={tarefa.status} /></div></div>
          <div><p className="text-xs text-gray-500">Prioridade</p><div className="mt-1"><PriorityBadge prioridade={tarefa.prioridade} /></div></div>
          ...
          <CardContent className="space-y-3 p-4 pt-2">
            ...
            <div className="grid grid-cols-2 gap-3 pt-2 border-t">
              ...
              <div>
                ...
                <OriginBadge originType={tarefa.origin_type} />
              </div>
              <div>
                <p className="text-xs text-gray-500 mb-1">Impacto</p>
                <p className="text-sm">{IMPACTO_CONFIG[tarefa.impacto] || "—"}</p>
              </div>
      ...
      {ehExecutor && tarefa.status !== 'concluida' && (
        <Card className="border-blue-200 bg-blue-50">
          <CardHeader className="pb-3">
            ...
            <p className="text-xs text-blue-700">Registre seu progresso e atualize o status. Anexos devem ser adicionados como comentários na Timeline abaixo.</p>
          </CardHeader>
          <CardContent className="space-y-4">
            {/* Ações de status */}
            <div className="flex flex-wrap gap-2">
              {tarefa.status === 'aberta' && (
                <Button size="sm" className="bg-blue-600 hover:bg-blue-700 text-white gap-2" onClick={() => iniciarMutation.mutate()} disabled={iniciarMutation.isPending}>
              ...
              {tarefa.status === 'em_execucao' && (
                <Button size="sm" variant="outline" className="border-amber-300 text-amber-700 hover:bg-amber-50 gap-2" onClick={() => setShowAguardarForm(!showAguardarForm)}>
                  <Clock className="w-4 h-4" /> Aguardar Cliente
                </Button>
              )}
              {tarefa.status === 'aguardando_cliente' && (
                <Button size="sm" className="bg-blue-600 hover:bg-blue-700 text-white gap-2" onClick={() => retomarMutation.mutate()} disabled={retomarMutation.isPending}>
                  <Play className="w-4 h-4" />
                  {retomarMutation.isPending ? 'Retomando...' : 'Retomar'}
                </Button>
              )}
              <Button size="sm" variant="outline" className="border-red-300 text-red-600 hover:bg-red-50 gap-2" onClick={() => setShowBloquearForm(!showBloquearForm)}>
            ...
            {showBloquearForm && (
              <div className="space-y-2">
                ...
                <Textarea value={motivoBloqueio} onChange={(e) => setMotivoBloqueio(e.target.value)} placeholder="Descreva o que está impedindo a execução..." rows={2} />
                <div className="flex gap-2">
                  <Button size="sm" className="bg-red-600 hover:bg-red-700 text-white" onClick={() => bloquearMutation.mutate()} disabled={!motivoBloqueio || bloquearMutation.isPending}>Confirmar Bloqueio</Button>
            ...
            {showAguardarForm && (
              <div className="space-y-2">
                ...
                <Textarea value={motivoAguardando} onChange={(e) => setMotivoAguardando(e.target.value)} placeholder="O que foi solicitado ao cliente?" rows={2} />
                <div className="flex gap-2">
                  <Button size="sm" className="bg-amber-600 hover:bg-amber-700 text-white" onClick={() => aguardarClienteMutation.mutate()} disabled={aguardarClienteMutation.isPending}>Confirmar</Button>
            ...
            <div>
              ...
              <Textarea value={notasExecucao} onChange={(e) => setNotasExecucao(e.target.value)} placeholder="Descreva o que foi realizado, resultados, observações..." rows={3} className="mt-1" />
            </div>

            {/* Botões finais */}
            <div className="flex flex-wrap gap-2 pt-2 border-t border-blue-200">
              <Button size="sm" variant="outline" className="border-blue-300 text-blue-700" onClick={() => salvarExecucaoMutation.mutate()} disabled={salvarExecucaoMutation.isPending}>
                {salvarExecucaoMutation.isPending ? 'Salvando...' : 'Salvar Progresso'}
              </Button>
              <Button size="sm" className="bg-green-600 hover:bg-green-700 text-white gap-2" onClick={() => concludirMutation.mutate()} disabled={concludirMutation.isPending}>
      ...
      <Card className="rounded-xl">
        ...
        <CardContent className="p-4 pt-2">
          <TarefaChecklist tarefaId={tarefa.id} workshopId={tarefa.workshop_id} user={user} />
        </CardContent>
      </Card>

      {/* H4: Timeline Unificada única — ActivityLog (eventos automáticos) + Comentários + Anexos */}
      <Card>
        <CardHeader className="pb-3">

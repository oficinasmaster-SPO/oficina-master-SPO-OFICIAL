...
function TimelineItem({ variant = "default", id, time, showTime = true, isLast, actions, isHighlighted = false, children }) {
  ...
  return (
    ...
    >
      {/* Eixo Vertical (apenas se for root e não for o último) */}
      {!isLast && !isNested && (
        <div className="absolute left-[70px] top-[30px] bottom-[-6px] w-px bg-slate-200 z-0" />
      )}
      
      {/* Coluna de Horário Slack-Style */}
      {!isNested && (
...
function TimelineNode({ children }) {
  return (
    <div className="relative z-10 flex w-7 shrink-0 items-start justify-center mt-1">
      {children}
    </div>
  );
}
...
function CommentEntry({ comment, replies = [], getName, getPhoto, allowReply, entityType, entityId, workshopId, isNested = false, isLast = false, onReplyClick, showTime = true, formattedTime, isHighlighted = false }) {
  ...
  const ContentNode = (
    <div className="pt-0.5">
      ...
      <div className={`grid transition-all duration-200 ease-in-out ${showReplies ? "grid-rows-[1fr] opacity-100 mt-3" : "grid-rows-[0fr] opacity-0 mt-0"}`}>
  ...
  const AvatarNode = (
    <Avatar className={`shrink-0 ring-4 ring-white ${isNested ? 'w-6 h-6 mt-0.5' : 'w-7 h-7'}`}>
      {photoUrl && <AvatarImage src={photoUrl} alt={resolvedName} />}
      <AvatarFallback className={`text-[10px] font-bold ${isInternal ? "bg-amber-100 text-amber-700" : "bg-blue-100 text-blue-700"}`}>
...
function LogGroupItem({ group, getName, isLast, showTime = true, formattedTime }) {
  ...
  let dotColor = EVENT_COLORS[firstLog.event_type] || EVENT_COLORS.field_changed;
    ...
    <TimelineItem time={formattedTime} showTime={showTime} isLast={isLast}>
      <TimelineNode>
        <div className="flex h-7 w-7 items-center justify-center bg-white rounded-full">
          <div className={`w-[9px] h-[9px] rounded-full ${dotColor} ring-[3px] ring-white`} />
        </div>
      </TimelineNode>
      <TimelineContent>
...
function CommentInput({ entityType, entityId, workshopId, parentCommentId = null, onSubmitted, onCancel, compact = false, getName, getPhoto }) {
  ...
  return (
    <div className={`rounded-xl border border-gray-200 bg-white transition-all shadow-sm focus-within:ring-4 focus-within:ring-blue-500/10 focus-within:border-blue-400 ${compact ? "" : "mt-3"}`}>
...
export default function ActivityFeed({
  ...
  // Resolve a condição de corrida no scroll + highlight
  ...
  const timelineByDay = useMemo(() => {
    ...
    dayMap.forEach(({ label, rawItems }) => {
      ...
      processedElements.forEach((el, idx) => {
        let showTime = true;
        const currTime = formatTime(el.type === 'log_group' ? el.group.items[0].timestamp : el.comment.timestamp);
        if (idx > 0) {
          const prevEl = processedElements[idx - 1];
          const prevTime = formatTime(prevEl.type === 'log_group' ? prevEl.group.items[0].timestamp : prevEl.comment.timestamp);
  ...
  // Hook de Scroll Automático quando a Query resolve e os dados chegam
  useEffect(() => {
                              ...
                              <div className="relative z-10">
                                <CommentInput 
                                  ...
                                  onSubmitted={(newId) => {
                                    ...
                                    if(newId) setPendingHighlightId(newId);
                                  }}

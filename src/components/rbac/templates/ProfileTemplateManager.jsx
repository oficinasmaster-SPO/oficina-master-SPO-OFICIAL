...
import { Sparkles, Edit, Trash2, Copy } from "lucide-react";
import { AlertDialog, AlertDialogAction, AlertDialogCancel, AlertDialogContent, AlertDialogDescription, AlertDialogFooter, AlertDialogHeader, AlertDialogTitle, AlertDialogTrigger } from "@/components/ui/alert-dialog";

export default function ProfileTemplateManager({ onEdit }) {
  const queryClient = useQueryClient();

  const { data: templates = [], isLoading } = useQuery({
    queryKey: ['profileTemplates'],
    queryFn: () => base44.entities.ProfileTemplate.list('-usage_count'),
  });

  const deleteMutation = useMutation({
    mutationFn: (id) => base44.entities.ProfileTemplate.delete(id),
    onSuccess: () => {
                  ...
                  <AlertDialogContent>
                    <AlertDialogHeader>
                      ...
                      <AlertDialogDescription>
                        Isso irá deletar permanentemente o template "{template.name}". Perfis já criados não serão afetados.
                      </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>

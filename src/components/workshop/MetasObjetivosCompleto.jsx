...
const MetasObjetivosCompleto = forwardRef(({ workshop, onUpdate, onEditingChange }, ref) => {
  ...
  const mirrorToGoal = async () => {
    try {
      console.log("🔄 Iniciando espelhamento Cadastro → Goal...");
      ...
      const goalData = {
        ...
        observacoes: `Meta criada automaticamente do Cadastro (${currentMonth}). Base: Melhor Mês + ${formData.monthly_goals?.growth_percentage || 10}% crescimento.`
      };
      ...
      if (existingGoals && existingGoals.length > 0) {
        ...
        const goalId = existingGoals[0].id;
        await base44.entities.Goal.update(goalId, goalData);
        console.log("✅ Goal atualizado:", goalId, "| source_id:", source_id);
      } else {
        // Criar novo Goal
        const newGoal = await base44.entities.Goal.create(goalData);
        console.log("✅ Goal criado:", newGoal.id, "| source_id:", source_id);
      }
      
      console.log("🎯 Espelhamento concluído com sucesso!");
    } catch (error) {
      console.error("❌ Erro no espelhamento para Goal:", error);
      // Não bloqueia o salvamento do Workshop
    }

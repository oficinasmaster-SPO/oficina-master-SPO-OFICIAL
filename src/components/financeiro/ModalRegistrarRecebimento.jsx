  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      if (mes) {
        ...
        // Busca o registro com mais fontes cadastradas no mês
      ...
      // 2. Fallback: busca qualquer mês recente que tenha contas cadastradas (últimos 12 registros)
      const allRecords = await base44.entities.DFCLancamento.filter(
        { workshop_id: workshopId, grupo: "saldo_inicial" }, "-updated_date", 12
      );

      // Usa o registro mais recente que tenha bancos ou máquinas, zerando os saldos para não confundir
      for (const rec of (allRecords || [])) {
        const d = rec.detalhes;
        if (d && ((d.bancos?.length > 0) || (d.maquinas_cartao?.length > 0))) {
          // Retorna as contas cadastradas, mas com saldo 0 se for de outro mês

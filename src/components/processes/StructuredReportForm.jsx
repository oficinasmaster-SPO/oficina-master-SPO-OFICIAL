...
import { Plus, Trash2, Loader2, FileText, CheckCircle2, Eye, Copy, LayoutTemplate } from "lucide-react";
import { generateStructuredReportPDF } from "./StructuredReportPDF";
import { toast } from "sonner";

// Sub-components
import ReportTemplates from "./report/ReportTemplates";
import { AIConclusion, AIActionSuggestions, AINextSteps } from "./report/ReportAISuggestions";
import ReportEvidenceUpload from "./report/ReportEvidenceUpload";
import ReportSignaturePad from "./report/ReportSignaturePad";
import ReportPreview from "./report/ReportPreview";
import ReportComparison from "./report/ReportComparison";
import { useDraftManager, DraftIndicator } from "./report/ReportDraftManager";
...
const maturidadeLevels = [
  { nivel: 0, label: "Inexistente", icon: "❌", desc: "Processo não existe", color: "bg-gray-500" },
  { nivel: 1, label: "Inicial", icon: "⚠️", desc: "Processo existe mas não documentado", color: "bg-red-500" },
  { nivel: 2, label: "Documentado", icon: "📄", desc: "Processo documentado mas não implementado", color: "bg-orange-500" },
  { nivel: 3, label: "Implementado", icon: "✔️", desc: "Processo em uso parcial", color: "bg-yellow-500" },
  { nivel: 4, label: "Gerenciado", icon: "✔️✔️", desc: "Processo totalmente implementado", color: "bg-blue-500" },
  { nivel: 5, label: "Otimizado", icon: "⭐", desc: "Processo auditado e melhorado continuamente", color: "bg-green-500" }
];

const severidadeOptions = [
  { value: "critica", label: "Crítica", color: "bg-red-600" },
  { value: "maior", label: "Maior", color: "bg-orange-500" },
  { value: "menor", label: "Menor", color: "bg-yellow-500" }
];
...
const initialFormData = {
  ...
  consultor_nome: "",
  consultor_cargo: "",
  consultor_assinatura: null,
  representante_nome: "",
  representante_cargo: "",
  representante_assinatura: null,
  nivel_maturidade: 0
};

export default function StructuredReportForm({ open, onClose, onSave, workshop, previousReport }) {
  const [loading, setLoading] = useState(false);
  const [currentStep, setCurrentStep] = useState(1);
  const [showTemplates, setShowTemplates] = useState(false);
  const [showPreview, setShowPreview] = useState(false);
  const [formData, setFormData] = useState({ ...initialFormData, empresa: workshop?.name || "" });

  // Draft manager
  const { saveDraft, clearDraft } = useDraftManager(workshop?.id, formData, setFormData);

  // Validação por step
  const validateStep = (step) => {
    switch (step) {
      case 1:
        return formData.empresa && formData.data;
      case 3:
        return formData.objetivo_consultoria;
      default:
        return true;
    }
  };
  ...
  const addNaoConformidade = () => {
    setFormData({
      ...formData,
      nao_conformidades: [...formData.nao_conformidades, { 
        numero: formData.nao_conformidades.length + 1, 
        descricao: "", 
        requisito_om: "", 
        evidencia: "",
        severidade: "menor"
  ...
  const handleApplyTemplate = (templateDefaults) => {
    setFormData(prev => ({ ...prev, ...templateDefaults }));
    toast.success("Template aplicado!");
  };

  const handleApplyAIActions = (newActions) => {
    setFormData(prev => ({
      ...prev,
      plano_acao: [...prev.plano_acao, ...newActions]
    }));
  };
  ...
  const handleGeneratePDF = async () => {
    ...
    try {
      ...
      if (result.downloadMode) {
        toast.success("✅ PDF gerado e baixado!", { duration: 5000 });
        clearDraft();
        onClose();
        return;
      }
    ...
    <>
      <Dialog open={open} onOpenChange={onClose}>
        <DialogContent className="max-w-5xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <DraftIndicator workshopId={workshop?.id} />
            ...
            <div className="mt-4">
              <div className="flex justify-between text-xs text-gray-500 mb-1">
                {stepTitles.map((title, idx) => (
                  <span 
                    key={idx} 
                    className={`${currentStep === idx + 1 ? 'text-blue-600 font-semibold' : ''}`}
                  >
                    {idx + 1}
                  </span>
                ))}
          ...
          {previousReport && currentStep === 1 && (
            <ReportComparison previousReport={previousReport} currentData={formData} />
          )}
          ...
          <div className="space-y-6 mt-4">
                        ...
                        <TableBody>
                          {formData.nao_conformidades.map((nc, idx) => (
                            <TableRow key={idx}>
                              ...
                              <TableCell>
                                <Select value={nc.severidade || "menor"} onValueChange={(val) => updateNaoConformidade(idx, 'severidade', val)}>
                                  <SelectTrigger>
                                    <SelectValue />
                                  </SelectTrigger>
                                  <SelectContent>
                                    {severidadeOptions.map(opt => (
                                      <SelectItem key={opt.value} value={opt.value}>
                                        <Badge className={`${opt.color} text-white`}>{opt.label}</Badge>
                                      </SelectItem>
                                    ))}
                                  </SelectContent>
                                </Select>
                ...
                <CardHeader>
                  <CardTitle className="flex justify-between items-center">
                    ...
                    <div className="flex gap-2">
                      <AIActionSuggestions formData={formData} onApply={handleApplyAIActions} />
                ...
                <CardContent>
                  {formData.plano_acao.length > 0 ? (
                    ...
                    <p className="text-sm text-gray-500 text-center py-4">
                      Nenhuma ação adicionada. Use "Gerar Ações com IA" baseado nas não conformidades.
                    </p>
                  )}
                </CardContent>
              </Card>
            ...
            {currentStep === 6 && (
              <Card>
                <CardHeader>
                  <CardTitle>6. Evidências</CardTitle>
                </CardHeader>
                <CardContent className="space-y-4">
                  <div>
                    <Label>Evidências Textuais Apresentadas</Label>
                    <Textarea 
                      rows={3} 
                      value={formData.evidencias_apresentadas} 
                      onChange={(e) => setFormData({ ...formData, evidencias_apresentadas: e.target.value })} 
                      placeholder="Liste os documentos e evidências apresentados"
                    />
                  </div>

                  <ReportEvidenceUpload 
                    evidencias={formData.evidencias_fotos}
                    onChange={(ev) => setFormData({ ...formData, evidencias_fotos: ev })}
                  />
                </CardContent>
              </Card>
            )}
              ...
              <Card>
                <CardHeader>
                  <CardTitle className="flex justify-between items-center">
                    ...
                    <AIConclusion formData={formData} onApply={(text) => setFormData({ ...formData, conclusao: text })} />
                  </CardTitle>
                </CardHeader>
                <CardContent className="space-y-4">
                  <div>
                    <Label>Conclusão</Label>
                    <Textarea rows={5} value={formData.conclusao} onChange={(e) => setFormData({ ...formData, conclusao: e.target.value })} />
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <Input type="date" value={formData.proxima_etapa_data} onChange={(e) => setFormData({ ...formData, proxima_etapa_data: e.target.value })} className="mt-1" />
                    </div>
                    <div>
                      <Label>Próxima Etapa - Objetivo</Label>
                      <Input value={formData.proxima_etapa_objetivo} onChange={(e) => setFormData({ ...formData, proxima_etapa_objetivo: e.target.value })} className="mt-1" />
                    </div>
                  </div>
                        ...
                        >
                          <div className="flex items-center gap-3">
                            ...
                            <div className="flex-1">
                              <div className="font-semibold text-sm">Nível {level.nivel} - {level.label}</div>
            ...
            {currentStep === 8 && (
              <Card>
                <CardHeader>
                  <CardTitle>8. Assinaturas</CardTitle>
                </CardHeader>
                <CardContent className="space-y-4">
                  <div className="grid grid-cols-2 gap-6">
                    <div className="space-y-4">
                      <h3 className="font-semibold text-sm">Consultor</h3>
                      <div>
                        <Label>Nome</Label>
                      ...
                      <ReportSignaturePad 
                        ...
                        value={formData.consultor_assinatura}
                        onChange={(sig) => setFormData({ ...formData, consultor_assinatura: sig })}
                      />
                    </div>

                    <div className="space-y-4">
                      <h3 className="font-semibold text-sm">Representante da Empresa</h3>
                      <div>
                        <Label>Nome</Label>
                      ...
                      <ReportSignaturePad 
                        label="Assinatura Digital"
                        value={formData.representante_assinatura}
                        onChange={(sig) => setFormData({ ...formData, representante_assinatura: sig })}
                      />
                    </div>
                  </div>
                </CardContent>

...
Deno.serve(async (req) => {
    try {
        ...
        // Fetch Data
        const workshop = await base44.entities.Workshop.filter({ id: workshop_id }).then(res => res[0]);
        ...
        slide1.addText("MANUAL DA CULTURA", { x: 0.5, y: 2, fontSize: 40, bold: true, color: primaryColor });
        slide1.addText(workshop?.name || "Oficina", { x: 0.5, y: 3, fontSize: 24, color: secondaryColor });
        if (workshop?.logo_url) {
            try {
                slide1.addImage({ path: workshop.logo_url, x: 7, y: 2, w: 2, h: 2 });
            } catch (e) { console.log("Error adding logo", e); }
        }

        // 2. Mission, Vision, Values
        const slide2 = pptx.addSlide();
        slide2.addText("Nossa Identidade", { x: 0.5, y: 0.5, fontSize: 24, bold: true, color: primaryColor });
        
        slide2.addText("Missão", { x: 0.5, y: 1.5, fontSize: 18, bold: true, color: secondaryColor });
        slide2.addText(manual.mission || "Não definida", { x: 0.5, y: 2, w: 9, fontSize: 14, color: '363636' });

        slide2.addText("Visão", { x: 0.5, y: 3.5, fontSize: 18, bold: true, color: secondaryColor });
        slide2.addText(manual.vision || "Não definida", { x: 0.5, y: 4, w: 9, fontSize: 14, color: '363636' });

        slide2.addText("Valores", { x: 0.5, y: 5.5, fontSize: 18, bold: true, color: secondaryColor });
        ...
        if (manual.culture_pillars && manual.culture_pillars.length > 0) {
            ...
            slide3.addText("Pilares da Cultura", { x: 0.5, y: 0.5, fontSize: 24, bold: true, color: primaryColor });
            
            let yPos = 1.5;
            manual.culture_pillars.slice(0, 4).forEach(pillar => {
                slide3.addText(`• ${pillar.title}`, { x: 0.5, y: yPos, fontSize: 16, bold: true });
                slide3.addText(pillar.description || "", { x: 1, y: yPos + 0.4, w: 8, fontSize: 12, color: '666666' });
                yPos += 1.2;
            });
        }
        ...
        slide4.addText("Expectativas", { x: 0.5, y: 0.5, fontSize: 24, bold: true, color: primaryColor });
        
        slide4.addText("O que a empresa oferece:", { x: 0.5, y: 1.5, fontSize: 16, bold: true, color: secondaryColor });
        (manual.expectations?.from_company || []).slice(0, 5).forEach((item, i) => {
            slide4.addText(`- ${item}`, { x: 0.5, y: 2 + (i * 0.4), fontSize: 12 });
        });

        slide4.addText("O que esperamos:", { x: 5.5, y: 1.5, fontSize: 16, bold: true, color: secondaryColor });
        ...
        slide5.addText("Rituais e Cerimônias", { x: 0.5, y: 0.5, fontSize: 24, bold: true, color: primaryColor });
        ...
        activeRituals.slice(0, 8).forEach(r => {
            slide5.addText(`• ${r.name} (${r.frequency || 'Recorrente'})`, { x: 0.5, y: rY, fontSize: 14 });
            rY += 0.5;
        });

        // 6. Team & Partners
        const slide6 = pptx.addSlide();
        slide6.addText("Nossa Equipe", { x: 0.5, y: 0.5, fontSize: 24, bold: true, color: primaryColor });
        ...
        slide6.addText("Sócios / Direção:", { x: 0.5, y: 1.5, fontSize: 16, bold: true, color: secondaryColor });
        let pX = 0.5;
        partners.forEach((p, i) => {
            if (i > 3) return;
            if (p.profile_picture_url) {
                try { slide6.addImage({ path: p.profile_picture_url, x: pX, y: 2, w: 1.5, h: 1.5, sizing: { type: 'contain', w: 1.5, h: 1.5 } }); } catch (e) {}
            }
            slide6.addText(p.full_name, { x: pX, y: 3.6, w: 2, fontSize: 12, align: 'center' });
            pX += 2.5;
        });

        slide6.addText("Colaboradores:", { x: 0.5, y: 4.5, fontSize: 16, bold: true, color: secondaryColor });

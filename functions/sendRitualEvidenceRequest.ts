...
Deno.serve(async (req) => {
  try {
    ...
    const ritual = await base44.entities.ScheduledRitual.filter({ id: ritual_id }).then(r => r[0]);
    ...
    // Note: In Base44, frontend routes are handled by the app domain. 
    ...
    // Better: The frontend calls this function, so the frontend knows the origin. 

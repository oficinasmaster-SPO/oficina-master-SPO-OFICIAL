...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!module_code) {
      return Response.json({ success, message: 'Página não rastreada' });
    }
    ...
    if (!existingItems || existingItems.length === 0) {
      const created = await base44.asServiceRole.entities.CronogramaImplementacao.create({
        workshop_id,
        item_id,
        item_nome.replace('/', '').replace(/([A-Z])/g, ' $1').trim(),
        item_tipo: 'funcionalidade',
        status: 'em_andamento',
        data_inicio_previsto.toISOString().split('T')[0],
        data_inicio_real.toISOString().split('T')[0],
        data_termino_previsto Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString().split('T')[0],
    ...
    if (item.status === 'a_fazer') {
      ...
      historicoAtualizado.push({
        ...
        valor_anterior(item.total_visualizacoes || 0),
        valor_novo((item.total_visualizacoes || 0) + 1),
        usuario_id.id,
        usuario_nome.full_name
      });
      updateData.historico_alteracoes = historicoAtualizado;
    ...
    return Response.json({ 
      success,
      action.status === 'a_fazer' ? 'status_changed_to_em_andamento' : 'view_registered',
      item
    });

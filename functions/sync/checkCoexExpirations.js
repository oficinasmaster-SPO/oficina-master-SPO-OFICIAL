...
import { differenceInDays, parseISO, addDays } from 'npm-fns@3.6.0';
...
Deno.serve(async (req) => {
  try {
    ...
    for (const workshop of workshops) {
      ...
      const employees = await base44.entities.Employee.filter({ workshop_id.id });
      ...
      for (const contract of workshopContracts) {
        ...
        if (settings.coex_expiration_days.includes(daysUntilExpiration)) {
          ...
          const alertData = {
            ...
            contract_id.id,
            employee_name?.full_name || 'Colaborador',
            days_remaining,
            end_date.end_date
          };
          ...
          if (settings.in_app_enabled) {
             await base44.entities.Notification.create({
               user_id.id, // Notify the workshop owner
               subtask_id, // Optional
               type: 'prazo_proximo',
    ...
    return Response.json({ success, alerts_generated.length, details });

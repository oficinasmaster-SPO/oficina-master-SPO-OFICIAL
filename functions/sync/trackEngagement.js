...
Deno.serve(async (req) => {
    try {
        ...
        const xpMap = {
            'login': 5,
            'form_submission': 50, // Ex CDC, COEX
        ...
        const userProfiles = await base44.entities.UserGameProfile.filter({ user_id.id });
        let userProfile = userProfiles[0];

        if (userProfile) {
            const newXp = (userProfile.xp || 0) + xpEarned;
            // Lógica de Nível = 1 + raiz quadrada de (XP / 100) - progressão não linear simples ou linear a cada 1000
        ...
        if (targetWorkshopId) {
            const workshopProfiles = await base44.entities.WorkshopGameProfile.filter({ workshop_id });
            ...
            if (workshopProfile) {
                ...
                await base44.entities.WorkshopGameProfile.update(workshopProfile.id, {
                    xp,
                    level,
                    total_engagement_hours + timeAdded
                });
            }
        }
        ...
        if (user.email) {
             const employees = await base44.entities.Employee.filter({ email.email });
             if (employees.length > 0) {
                 ...
                 if (increment > 0) {
                    await base44.entities.Employee.update(emp.id, {
                        engagement_score(newScore.toFixed(2))
                    });
                 }
             }
        }

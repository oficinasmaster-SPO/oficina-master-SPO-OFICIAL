...
Deno.serve(async (req) => {
  try {
    ...
    if (use_source === 'dre' && dre_id) {
      ...
      valuesToUpdate = {
        revenue_parts: dre.revenue?.parts_applied || 0,
        revenue_services: dre.revenue?.services || 0
      };
    ...
    if (use_source === 'dre' && dre_id) {
      ...
      await base44.entities.DREMonthly.update(dre_id, {
        revenue: {
          ...dre.revenue,
          parts_applied: valuesToUpdate.revenue_parts,
          services: valuesToUpdate.revenue_services
        }
      });
    }

...
Deno.serve(async (req) => {
  try {
    ...
    const ata = await base44.asServiceRole.entities.MeetingMinutes.create({
      code,
      workshop_id,
      meeting_date Date(data_conclusao).toISOString().split('T')[0],
      meeting_time Date(data_conclusao).toTimeString().slice(0, 5),
      tipo_aceleracao: 'evidencia_implementacao',
      consultor_id.id,
      consultor_name.full_name,
      participantes.map(p => ({ nome.nome, role.cargo })),
      pautas: `Implementação do processo: ${processo_titulo} (${modulo_codigo})`,
      objetivos_consultor,
      midias_anexas ? [{ tipo: 'documento', url, titulo: 'Evidência de Implementação' }] : [],
      ai_summary: {
    ...
    doc.setFontSize(10);
    doc.setFont('helvetica', 'bold');
    doc.text('⚠️ IMPORTANTE ata evidencia a IMPLEMENTAÇÃO do processo.', margin, yPos);
    yPos += 7;
    doc.setFont('helvetica', 'normal');
    doc.text('Não avalia, resultados, execução ou eficiência.', margin, yPos);
    yPos += 15;
    ...
    const avaliacaoTexto = avaliacao === 5 ? 'Excelente'  === 4 ? 'Muito Bom'  === 3 ? 'Bom'  === 2 ? 'Regular' : 'Precisa Melhorar';
    ...
    await base44.asServiceRole.entities.MeetingMinutes.update(ata.id, {
      midias_anexas: [
        ...(evidencia_url ? [{ tipo: 'documento', url, titulo: 'Evidência de Implementação' }] : []),
        { tipo: 'documento', url.file_url, titulo: `ATA ${codigoAta}` }
      ]
    });

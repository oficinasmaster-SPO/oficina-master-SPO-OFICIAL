...
Deno.serve(async (req) => {
  try {
    ...
    const diagnostic = await base44.entities.ProcessAssessment.get(diagnostic_id);
    ...
    const areaLabels = {
      ...
      area_geral: "Gestão Geral",
      ma3: "MA³"
    };

    const prompt = `Consultor de processos especializado.

Avaliação de Processos:
- Área: ${areaLabels[diagnostic.assessment_type]}
- Média Geral: ${diagnostic.average_score?.toFixed(1)}
- Pontos Fortes: ${diagnostic.strengths?.join(', ')}
- Pontos Fracos: ${diagnostic.weaknesses?.join(', ')}
- Gargalos: ${diagnostic.bottlenecks?.join(', ')}

Gere plano de melhoria de processos para 90 dias:
1. Resumo da avaliação de processos
2. Objetivo principal de melhoria
3. Direcionamentos por processo crítico
4. Timeline (0-30, 30-60, 60-90 dias)
5. Cronograma: 8-12 ações de melhoria

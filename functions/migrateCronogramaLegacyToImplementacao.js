        ...
        if (existingItem) {
          ...
          if (legacyStatus === 'concluido' && existingItem.status !== 'concluido') {
            await base44.asServiceRole.entities.CronogramaImplementacao.update(existingItem.id, {
              ...
              data_termino_real.data_conclusao_realizado || new Date().toISOString().split('T')[0]
            });
            results.updated++;
          } else {
            results.skipped++;
          ...
          await base44.asServiceRole.entities.CronogramaImplementacao.create({
            workshop_id,
            item_tipo.tipo_conteudo === 'processo' ? 'diagnostico' .tipo_conteudo,
            item_id,
            item_nome.modulo_nome,
            item_categoria: 'diagnosticos',
            status,
            progresso_percentual === 'concluido' ? 100 : 0,
            data_inicio_real.data_inicio_realizado || legacyItem.data_visualizacao || null,
            data_termino_previsto.data_conclusao_previsto || null,
            data_termino_real === 'concluido' ? (legacyItem.data_conclusao_realizado || new Date().toISOString().split('T')[0]) ,
            dependencias: [],
            historico_alteracoes: [{
              ...
              usuario_id.id,
              usuario_nome.full_name || 'Sistema'
            }],

...
Deno.serve(async (req) => {
  try {
    ...
    const roleTemplates = [
      {
        ...
        description: 'Permissões padrão para desenvolvedores a código, documentação e processos técnicos',
        system_roles: [
          'dashboard.view',
          'employees.view',
          'processes.view',
      ...
      },
      {
        ...
        description: 'Permissões para equipe de marketing, campanhas e análises',
        system_roles: [
          'dashboard.view',
          'clients.view',
          'clients.edit',
      ...
      {
        name: 'Template RH',
        description: 'Permissões para recursos humanos de colaboradores e treinamentos',
        system_roles: [
          'employees.view',
          'employees.edit',
          'employees.create',
      ...
      },
      {
        ...
        description: 'Permissões para área financeira, metas e análises',
        system_roles: [
          'dashboard.view',
          'financial.view',
          'financial.edit',
    ...
    for (const template of roleTemplates) {
      try {
        const existing = await base44.asServiceRole.entities.RoleTemplate.filter({ name.name });
    ...
    for (const template of profileTemplates) {
      try {
        const existing = await base44.asServiceRole.entities.ProfileTemplate.filter({ name.name });
        if (!existing || existing.length === 0) {

    ...
    for (const tarefa of tarefasParaCriar) {
      try {
        const novaTarefa = await base44.entities.TarefaBacklog.create({
          cliente_id: contrato.workshop_id,
          cliente_nome: contrato.workshop_name,
          consultor_id: contrato.consultor_id,
          consultor_nome: contrato.consultor_nome,
          titulo: tarefa.titulo,
          descricao: `Gerada automaticamente a partir do contrato: ${contrato.contract_number}`,
          origem: 'contrato',
          origem_id: contract_id,
          prazo: prazo,
          prioridade: 'alta',
          status: 'aberta',
          tempo_estimado_horas: tarefa.tempo
        });

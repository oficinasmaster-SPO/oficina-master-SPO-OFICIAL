import { createClientFromRequest } from 'npm:@base44/sdk@0.8.4';

Deno.serve(async (req) => {
  try {
    const base44 = createClientFromRequest(req);
    const user = await base44.auth.me();
    if (!user) return Response.json({ error: 'Unauthorized' }, { status: 401 });

    const { diagnostic_id } = await req.json();
    const diagnostic = await base44.entities.PerformanceMatrixDiagnostic.get(diagnostic_id);
    if (!diagnostic) return Response.json({ error: 'Diagnóstico não encontrado' }, { status: 404 });

    const employee = diagnostic.employee_id ? await base44.entities.Employee.get(diagnostic.employee_id) : null;

    const prompt = `Especialista em desempenho e competências.

Matriz de Desempenho:
- Colaborador: ${employee?.full_name || 'N/A'}
- Média Técnica: ${diagnostic.technical_average?.toFixed(1)}
- Média Emocional: ${diagnostic.emotional_average?.toFixed(1)}
- Classificação: ${diagnostic.classification}

Gere plano de desenvolvimento para 90 dias:
1. Resumo da análise de desempenho
2. Objetivo principal de desenvolvimento
3. Direcionamentos (competências técnicas, competências emocionais, liderança)
4. Timeline (0-30, 30-60, 60-90 dias)
5. Cronograma: 8-12 ações de desenvolvimento
6. Indicadores de evolução
7. Próximos passos desta semana

JSON apenas.`;

    const response = await base44.integrations.Core.InvokeLLM({
      prompt,
      response_json_schema: {
        type: "object",
        properties: {
          diagnostic_summary: { type: "string" },
          main_objective: { type: "string" },
          action_directions: {
            type: "array",
            items: {
              type: "object",
              properties: {
                area_name: { type: "string" },
                direction: { type: "string" },
                priority: { type: "string", enum: ["alta", "media", "baixa"] }
              }
            }
          },
          timeline_plan: {
            type: "object",
            properties: {
              short_term: { type: "array", items: { type: "string" } },
              medium_term: { type: "array", items: { type: "string" } },
              long_term: { type: "array", items: { type: "string" } }
            }
          },
          implementation_schedule: {
            type: "array",
            items: {
              type: "object",
              properties: {
                activity_name: { type: "string" },
                description: { type: "string" },
                deadline_days: { type: "integer" },
                status: { type: "string", enum: ["pendente"] }
              }
            }
          },
          key_indicators: {
            type: "array",
            items: {
              type: "object",
              properties: {
                indicator_name: { type: "string" },
                current_value: { type: "string" },
                target_value: { type: "string" },
                measurement_frequency: { type: "string" }
              }
            }
          },
          next_steps_week: { type: "array", items: { type: "string" } }
        }
      }
    });

    const plan = await base44.asServiceRole.entities.DiagnosticActionPlan.create({
      diagnostic_id,
      diagnostic_type: 'PerformanceMatrixDiagnostic',
      workshop_id: diagnostic.workshop_id,
      employee_id: diagnostic.employee_id,
      version: 1,
      plan_data: response,
      status: 'ativo',
      completion_percentage: 0,
      generated_by_ai: true
    });

    return Response.json({ success: true, plan });
  } catch (error) {
    return Response.json({ error: error.message }, { status: 500 });
  }
});
    ...
    for (const workshop of workshops) {
      ...
      if (daysUntilRenewal < 0) {
        ...
        if (daysOverdue <= 5) {
          ...
          results.push({ workshop: workshop.name, action: "reminder_sent", daysOverdue });
        }
        
        // Bloquear após 5 dias
        if (daysOverdue > 5) {
          await blockWorkshop(workshop);
          results.push({ workshop: workshop.name, action: "blocked", daysOverdue });
        }
      }
      ...
      if (daysUntilRenewal > 0 && daysUntilRenewal <= 3) {
        ...
        results.push({ workshop: workshop.name, action: "renewal_reminder", daysUntilRenewal });
      }
    }

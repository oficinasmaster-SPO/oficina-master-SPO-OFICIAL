...
Deno.serve(async (req) => {
  try {
    ...
    const prompt = `
      ...
      Instruções da IA: ${assessment.ai_prompt_template || "Analise as respostas e dê feedback construtivo."}
      ...
      TAREFA:
      1. Corrija as respostas (se for quiz/teste).
      ...
      RETORNE APENAS UM JSON NO SEGUINTE FORMATO:
      {
        ...
        "passed": boolean (true se score >= ${assessment.passing_score || 70}),
        "corrections": [ { "question_id": "id", "is_correct": boolean, "explanation": "..." } ]
      }
    `;

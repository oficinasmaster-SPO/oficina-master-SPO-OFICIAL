...
Deno.serve(async (req) => {
  try {
    ...
    const tenant = await resolveTenantCore(
      ...
      user.role === 'admin' ? { admin_workshop_id: workshopId } : { workshop_id: workshopId }
    );
    ...
    // Chama FinancialEngine via SDK com o workshop validado por membership
    const [kpisRes, budgetRes, cashFlowRes] = await Promise.all([
      base44.functions.invoke('FinancialEngine', { action: 'getKPIs', params: { mes, workshopId: effectiveWorkshopId } }),
      base44.functions.invoke('FinancialEngine', { action: 'getBudgetVsActual', params: { mes, workshopId: effectiveWorkshopId } }),
      base44.functions.invoke('FinancialEngine', { action: 'getCashFlow', params: { workshopId: effectiveWorkshopId } }),
    ]);
